...
def env_var_path():
    try:
        spath = r"ffmpeg"  # Source Path
        dpath = r"C:\\ffmpeg"  # Destination Path

        cmd.run(["robocopy", spath, dpath, "/E", "/NFL", "/NDL", "/NJH", "/NJS", "/nc", "/ns", "/np"], check=True)

        # Setting up the Environment Variable for the ffmpeg video tool.!
...
if __name__ == "__main__":
    ...
    root = tk.Tk()
    root.title("Money-Ape : Pytube Pro")
